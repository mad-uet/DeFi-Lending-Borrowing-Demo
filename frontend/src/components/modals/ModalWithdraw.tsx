...
export default function ModalWithdraw({ supply, onClose }: ModalWithdrawProps) {
  ...
  const usdValue = amount
    ? (parseFloat(amount) * (mockPrices[supply.asset.symbol] || 0)).toFixed(2)
    : '0.00';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">Withdraw {supply.asset.symbol}</h2>
          <button
            onClick={onClose}
        ...
        <button
          ...
          className="w-full px-6 py-3 bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-colors"
        >
          {status.status === 'idle' ? 'Withdraw' : status.status === 'success' ? '✓ Success' : 'Processing...'}
        </button>
      </div>
    </div>
  );
